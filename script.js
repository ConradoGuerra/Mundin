...
const geradorDeTabelas = (nomeDaTabelaBuscada, nomeDaTabelaGerada) => {
  const file = reader.readFile(`./${nomeDaTabelaBuscada}.xlsx`);
  ...
  for (let rodadas = 1; rodadas < Object.keys(jogos).length; rodadas++) {
    ...
    for (let teams = 0; teams < Object.keys(jogos).length / 2; teams++) {
      ...
      rivaisDoClube.jogos.map(partida => {
        ...
        if (jogos[clubeSelecionado].jogosMarcados.includes(partida.rival))
          return;
        ...
        function verifadorDeJogosRemanescentes(arrayDosClubes) {
          ...
          for (let c = 0; c < arrayDosClubes.length; c++) {
            ...
            const inimigos = arrayDosClubes.filter(
              inimigo => inimigo !== clube
            );
        ...
        if (rodadas === Object.keys(jogos).length - 1) {
          ...
          if (!jogos[clubeSelecionado].comeca) {
            if (
              ...
              primeiraLevaDeJogos.push(
                `${clubeSelecionado} x ${partida.rival}`
              );
              partidaAgendada = 1;
              matches.splice(matches.indexOf(clubeSelecionado), 1);
              matches.splice(matches.indexOf(partida.rival), 1);
              ...
              primeiraLevaDeJogos.push(
                `${partida.rival} x ${clubeSelecionado}`
              );
        ...
        if (rodadas === Object.keys(jogos).length - 2) {
          ...
          if (jogos[clubeSelecionado].casa === 0) {
            primeiraLevaDeJogos.push(`${clubeSelecionado} x ${partida.rival}`);
  ...
  reader.writeFile(workbook, `${nomeDaTabelaGerada}.xlsx`);
};

geradorDeTabelas("Matriz_norte", "Tabela_norte");
geradorDeTabelas("Matriz_sul", "Tabela_sul");
